...
def getAccountByYear(dbname,tablename,start_period,end_period,accountname):
    ...
    for yr in range(start_period,end_period+1):
        ...
        query = "SELECT distinct report_date,Januari,Februari,Maret,April,Mei,Juni,Juli,Agustus,September,Oktober,November,Desember from {} where strftime('%m',report_date)='12' and trim(Account) in ({}) and strftime('%Y',report_date)='{}' order by report_date asc".format(tablename,filter,yr)
        if yr==2023:
            query="SELECT distinct report_date,Januari,Februari,Maret,April,Mei,Juni,Juli,Agustus,September,Oktober,November,Desember from {} where trim(Account) in ({}) and  strftime('%Y',report_date)='{}' and  strftime('%m',report_date)='09' order by report_date asc".format(tablename,filter,yr)
